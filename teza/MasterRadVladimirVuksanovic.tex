...
Kompilacija velikih projekata može da traje baš dugo, tako da se ovaj pristup ne isplati uvek.
...
\begin{itemize}
  \item Evaluiranje vrednosti izraza --- Kada je proces zaustavljen na nekom mestu u kodu, korisnik želi da proveri vrednosti promenljivih. Neki debageri osim toga podržavaju i izračunavanje proizvoljnih izraza koji uključuju promenljive i funkcije programa koji se debaguje.
...
Dovoljno je da pritisne odgovarajuće dugme ili na neki način izmeni neki drugi element.
...
Većina informacija za debagovanje nastaju u prednjem delu kompjalera, pošto je on zadužen za analiziranja izvornog koda, i prosleđuju se kroz ostale faze kompilacije gde se usput dopunjuju novim podacima kada oni postanu dostupni (npr. lokacije promenljivih za vreme izvršavanja). % do izvršnog fajla.
...
% a u nekim slučajevima je čak potpuno nemoguće.
a nekada je to čak potpuno nemoguće.
U slučaju da je moguće izabrati između nepotpunih informacija ili informacije koje ne odgovaraju u potpunosti izvornom kodu, bolje je izabrati prvu opciju.
...
% TODO: ovo treba nekako vezati sa prethodnim poglavljem
...
Svaka etiketa i atribut imaju numeričku vrednost koja služi za zapisivanje u računaru i ime u obliku niske koje služi čoveku za lako čitanje podataka.
...
Bitna stvar pilikom dodavanja etiketa i atributa je da im se dodele 
% Bitno je samo da im dodeli 
numeričke vrednosti koje nisu već deo standarda i poželjno bi bilo da se takođe ne koriste od strane drugih proizvođača kompajlera.
...
Napravljen je kao skup ponovo upotrebljivih komponenti koje ekstenzivno koriste biblioteke iz LLVM projekta kao što su parser izraza biblioteke clang ili disasembler biblioteke LLVM \cite{lldb}.
...
Ne moraju uvek da budu dostupni svi ti podaci, a nekada nisu ni svi dostupni.
...
Glavni predmet rada je optimizacija izdvajanja koda koja štedi memoriju programa tako što pomera ponovljene delove koda u funkciju i uklanja duplikate.
...
% Postoji nekoliko stvari koje bi mogle da se poboljšaju u odnosu na predstavljenu implementaciju.
...
Iako je to dovoljno za sada, u budućnosti kada implementacija \verb|GlobalIsel| bude prešla u širu upotrebu biće potrebno podržati i tu implementaciju.
