...
\apstr{%
% Skoro sav moderan softver napisan je u višem programskom jeziku što znači da je potrebno kompilirati ga do izvršnog koda kako bi mogao da se koristi.
...
Debager se oslanja na informacije za debagovanje koje kompilator generiše pored mašinskog koda, ali te informacije je teško održati što se više krajnji k\^od razlikuje od početnog.
Ovaj problem se susreće prilikom optimizacije izdvajanja koda u kompilatoru LLVM.
...
Posledica manje količine koda je što nema dovoljno prostora za zapis lokacija u izvornom kodu koje čine bitan deo informacija za debagovanje.
Lokacije se zapisuju unutar tabele linija i može da postoji po jedna za svaku instrukciju programa.
Smanjenjem broja instrukcija prilikom izdvajanja koda, izgubi se mesto za skladištenje odgovarajućih lokacija.
...
Rešenje je implementirano kao nadogradnja alata iz infrastrukture LLVM, ali se ideja može primeniti na druge kompilatore i debagere.
}

% Ključne reči na srpskom jeziku (u odabranom pismu)
\kljucnereci{izdvajanje koda, kompilator, debager, informacije za debagovanje, projekat LLVM, kompilator LLVM, debager LLDB}
...
Postoje dva pristupa prevođenju viših jezika: \emph{interpretiranje} i \emph{kompiliranje}.
Interpretiranje podrazumeva postojanje odvojenog programa, \emph{interpretatora}, koji kao ulaz prima program napisan na višem programskom jeziku i ulazne podatke i u toku izvršavanja prevodi prosleđeni program naredbu po naredbu.
Alternativni pristup je prevesti izvorni k\^od u izvršni fajl pre njegovog pokretanja.
Tako dobijen program može da se izvršava samostalno bez dodatnih alata. % i to uglavnom brže nego ukoliko bi bio interpretiran.
Za taj posao zadužen je \emph{kompilator}.
%
Pored glavnog zadatka da prevede program napisan na višem jeziku u sematički ekvivalentan program za neku mašinu, drugi bitan zadatak kompilatora je da usput transformiše taj program tako da ima bolje performanse.
Neke od prednosti ovog pristupa su brže izvršavanje, manji trošak memorije i prepoznavanje jezičkih grešaka pre izvršavanja.
% Ovaj rad se fokusira na pristup kompiliranjem.

% Znacaj informacija za debagovanje
Napisan i preveden program može da ima propuste u svojoj implementaciji. Zbog toga, bitan deo razvoja softvera je rešavanje grešaka u napisanom programu.
...
Što se program više razlikuje od izvornog koda postaje teže, ako ne i nemoguće, ažurirati informacije za debagovanje tako da i dalje opisuju izvorni k\^od.
...
Između ostalog, koračanje po programu potpuno preskače instrukcije bez lokacije i nije moguće postaviti tačku prekida na njih.
...
% Kompilatori, struktura, optimizacije, kompilator LLVM
...
% Implementacija rešenja: za kompilator, za debager
...
% % Kompilirani i interpretirani jezici
% % Prednosti kompiliranih programskih jezika
...
% Kompilatori su alati koji rade taj posao.
% % Ključnu ulogu imaju kompilatori
...
Kako računari još uvek izvršavaju samo mašinski k\^od, nastali su posebni alati, asembleri, koji vrše jednostavne zamene mnemonika i operanada sa odgovarajućim mašinskim kodom.
...
% Zbog toga postoje brojna istraživanja na temu kompilatora i njihove implementacije.
...
Uz to, neophodno svojstvo je da i konačni program bude što efikasniji.
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.85\textwidth]{assets/compiler_structure.png}
...
% https://en.wikipedia.org/wiki/Optimizing_compiler
% https://poincare.matf.bg.ac.rs/~filip/kk/materijali/kk.pdf
Optimizacije se u odnosu na opseg koda koji obrađuju mogu podeliti na:
\begin{description}
  \item[Lokalne optimizacije] --- Rade na nivou jednog osnovnog bloka (eng.~{\em basic block}). Ovo su najjednostavnije optimizacije zato što ne obrađuju instrukcije skoka. % Primer ovog tipa optimizacije je sažimanje konstanti.
  \item[Globalne (intraproceduralne) optimizacije] --- Operišu na nivou pojedinačnih funkcija. Većina lokalnih optimizacija se može modifikovati da radi globalno. % U ovu grupu spada % globalna propagacija konstanti
  \item[Interproceduralne optimizacije] --- Rade na nivou čitavog programa. One istovremeno obrađuju više funkcija, potencijalno iz različitih kompilacionih jedinica. % Ovde pripada umetanje koda (eng.~{\em inlining}).
\end{description}
% Posmatranjem veće količine koda moguće je raditi naprednije optimizacije.
% Primeri lokalne, globalne i interproceduralne optimizacije su:
...
\begin{description}
  ...
  \item[Globalna numeracija vrednosti (eng.~{\em global value numbering})] --- Neki izrazi ili delovi izraza se mogu pojaviti na više mesta u kratkom rasponu u okviru funkcije. Ukoliko su izračunati jednom, nije potrebno računati ih ponovo ispočetka već je moguće obezbediti da prethodni rezultat ostane dostupan u memoriji dok nije ponovo potreban.
  ...
  \item[Umetanje koda (eng.~{\em inlining})] --- Pozivi funkcija su skupa operacija zato što menjaju kontrolu toka izvršavanja i zahtevaju dodatne instrukcije za čuvanje i učitavanje vrednosti registara. Zbog toga je poželjno zameniti pozive kratkih funkcija sa njihovom definicijom. Time se izbegava cena poziva o trošku veće količine memorije za čuvanje programa.
\end{description}
...
Inicijalno, naziv LLVM je bio akronim za "virtuelna mašina niskog nivoa" (eng.~{\em Low Level Virtual Machine}).
...
Mnoge firme koriste svoje verzije kompilatora LLVM bilo za podršku neke arhitekture ili kao osnovu za novi programski jezik. % TODO cite
...
Bitan korak koji se izvršava pre leksiranja je pretprocesiranje u okviru kog se obrađuju direktive poput \verb|#include| i \verb|#define|.
...
\begin{listing}[!ht]
\begin{minted}[fontsize=\small,breaklines,tabsize=4]{c}
  #include <stdio.h>
...
Primer korišćenja alata \verb|opt| za primenu optimizacija \texttt{mem2reg} i \texttt{instnamer} izgleda ovako:
% primer opt komanda
% \begin{listing}[!ht]
\begin{minted}[]{text}
...
U terminologiji projekta LLVM, ona se naziva MIR (eng.~{\em Machine Intermediate Representation}).
...
Veliki i kompleksan korak u prevođenju koda je prevođenje sa mašinski nezavisne na mašinski zavisnu reprezentaciju.
...
% https://link.springer.com/chapter/10.1007/978-3-540-72521-3_21
% https://www.llvm.org/devmtg/2011-11/Olesen_RegisterAllocation.pdf
% https://dl.acm.org/doi/10.1109/SCCC.2012.29
Poslednji trag LLVM međukoda koji je opstao u programu, a da je potrebno da se eliminiše, su virtuelni registri.
...
% Prvo se vrši dodeljivanje registara koji su implicitno određeni od strane instrukcija.
% Dalje dodeljivanje se radi heuristički pošto je optimalan algoritam NP-kompletan \cite{bouchez2007regalloc}.
Pre dolaska u ovu fazu, već su dodeljeni registri koji su implicitno definisani od strane instrukcija ili konvencije poziva funkcije.
Za dalje dodeljivanje registara se koriste različite heuristike pošto je optimalan algoritam NP-kompletan \cite{bouchez2007regalloc}.
Jedna od njih je gramziva (eng.~{\em greedy}) strategija koja daje prvenstvo dodeljivanju registara promenljivama sa dužim životnim vekom.
Kada nije moguće zadržati sve potrebne promenljive u registrima, dodaju se nove instrukcije za čuvanje i učitavanje tih vrednosti u radnu memoriju.

Dobijeni objekat \verb|SelectionDAG| predstavlja instrukcije potpuno ispravnog MIR programa.
...
Proces reprodukovanja, analiziranja i popravljanja grešaka u programu naziva se \emph{debagovanje}.
...
\begin{description}
  ...
  \item[Izvršavanje korak po korak] --- Ovo predstavlja najčešći i najjednostavniji vid kretanja kroz program. Omogućava kontrolisano izvršavanje samo određenog dela koda. Korak može da označava jednu liniju izvornog koda, funkciju, instrukciju...
...
Većina informacija za debagovanje nastaju u prednjem delu kompilatora, pošto je on zadužen za analiziranja izvornog koda, i prosleđuju se kroz ostale faze kompilacije gde se usput dopunjuju novim podacima kada oni postanu dostupni (npr. lokacije promenljivih za vreme izvršavanja). % do izvršnog fajla.
...
Informacije za debagovanje ne modifikuju nijedan od prethodnih segmenata već definišu nove kako ne bi uticali na postojeće podatke.
...
% Mogucnost koriscenja razlicitih kombinacija kompilatora i debagera
...
% Iako moze da se zameni kompilator, ne mora da znaci da ce isti podaci biti generisani niti da ce ih debager u istoj meri korstiti
...
\begin{description}
  ...
  % \item[\texttt{.debug\_rnglists}] --- Prekidni opsezi adresa. Koriste se iz \verb|.debug_info| sekcije kada opseg koji je potrebno opisati nije neprekidan.
  \item[\texttt{.debug\_abbrev}] --- Skraćenice za zapisivanje informacija u \verb|.debug_info| sekciji. Koristi se za kompresovanje podataka. % Umesto da se ispred svakog atributa napise njegov tip, ovde se definiše etiketa i redosled svih atributa u okviru jedinice i dodeljuje se skraćenica za zapis te jedinice.
  \item[\texttt{.debug\_str}] --- Tabela niski koje se koriste u \verb|.debug_info| sekciji.
\end{description}

% etikete i atributi
Podaci u sekciji \verb|.debug_info| su predstavljeni pomoću \emph{osnovnih jedinica} (eng.~{\em Debug Information Entry, DIE}) organizovanih u drvoliku strukturu.
...
Ovi podaci nisu ažurni u toku izvršavanja procesa već se računaju po potrebi kada se debagovan proces zaustavi.
...
\emph{Izdvajanje koda} (eng.~{\em outlining}) je interproceduralna kompilatorska optimizacija koja smanjuje količinu memorijskog prostora koji zauzima program, ali pritom potencijalno povećava njegovo vreme izvršavanja.
...
% Nije moguće uvek pronaći sve semantički iste segmente, već se poredi sličnost u internoj reprezentaciji kompilatora.
U nekim slučajevima moguće je da se ubrza i vreme potrebno za kompilaciju programa \cite{grune2012design}.
% Ovo nije zamena za definisanje novih funkcija u izvornom kodu koje implementiraju čestu operaciju, već je dopuna koja radi dodatno izdvajanje na nivou bližem mašinskom kodu.
...
\begin{listing}[!ht]
\begin{minted}[xleftmargin=20pt, linenos, tabsize=4, fontsize=\small]{c}
int global;
...
\begin{listing}[!ht]
% \begin{minted}[tabsize=4, fontsize=\footnotesize, breaklines]{asm}
% main:                                   # @main
% .Lfunc_begin1:
...
\begin{minted}[tabsize=4, fontsize=\footnotesize, breaklines]{llvm}
; ModuleID = 'outline.ll'
...
Ova optimizacija predstavlja suprotan proces poznatijoj optimizaciji, umetanju koda.
...
% U programskim jezicima C i C++ programer moze da nagovesti kompilatoru da umetne neku funkciju koristeći ključnu reč \verb|inline|.
% Kompilator ovo tretira kao sugestiju, tako da moze da izabere da ne izvrši optimizaciju ukoliko proceni da dobitak nije dovoljno veliki da nadomesti cenu ili ako nije moguće umetnuti funkciju (npr. jer se toj funkciji pristupa preko pokazivača).
...
% Moguće je, i poželjno, ručno izdvojiti često ponovljen segment izvornog koda u funkciju, ali na nivoima bližim mašinskom kodu taj posao je bolje prepustiti kompilatoru.
Izdvajanje koda nije zamena za definisanje novih funkcija u izvornom kodu koje implementiraju operacije koje se često ponavljaju, već je dopuna koja radi dodatno optimizovanje za memoriju na nivou bližem mašinskom kodu.
% To omogućava izdvajanje jedinica koje su manje od naredbi.
Ukoliko kompilator podržava ovu optimizaciju, njegov zadatak je da izabere delove koda koje je poželjno izdvojiti tako da se postigne što veća ušteda memorije.
...
\begin{itemize}
  ...
  \item Ilegalne instrukcije su one čije izdvajanje je zabranjeno zato što bi moglo da promeni semantiku programa ili zato što zahteva dodatne provere koje nisu podržane. To uključuje, na primer, pozive funkcija ili instrukcije vezane za obradu grešaka. 
...
\subsection{Specifičnosti implementacije izdvajanja koda na mašinski zavisnom međukodu}
...
Potrebno je dopuniti nekoliko delova opšteg algoritma.
Za kreiranje reprezentacije pogodne za pretragu podniski potrebno je definisati algoritam za heširanje instrukcija.
Izbor kandidata za izdvajanje zahteva definisanje metrike za procenu dobitka i neophodan je algoritam za izdvajanje tih kandidata u funkcije.
U nastavku će biti objašnjen svaki od ovih delova.
...
Svi kandidati se zamenjuju pozivom ka generisanoj funkciji bez potrebe za dodavanje bilo kakvih novih instrukcija.
% odrzava se liveness

\subsection{Specifičnosti implementacije izdvajanja koda na mašinski nezavisnom međukodu}
...
U nastavku će, kao i za prethodnu implementaciju, biti objašnjeni algoritam za heširanje instrukcija, metrika za procenu dobitka i način generisanja izdvojenog koda.
...
U idealnom slučaju, svaki izraz i naredba u izvornom kodu bi trebalo da ima odgovarajuću lokaciju, ali to nije slučaj pogotovu u optimizovanom kodu.
...
Kako izdvajanje funkcija ostavlja k\^od sa manje instrukcija nego sto je bilo pre toga, nije moguće smestiti potrebne informacije u tabelu linija. % ne postoji dovoljno mesta da se smeste potrebne informacije.
Jednoj instrukciji bi u tom slučaju moralo da odgovara više mogućih lokacija, što nije dozvoljeno.
...
% Jedino što je potrebno uraditi je dodati deklaraciju u fajlu \verb|Intrinsics.td| sa nazivom i parametrima.
...
Od uvedenih podataka za potrebe ovog rada, za instrukciju \verb|dbg.outlined| i vezu između metapodatka \verb|outline_id| i instrukcija za koje su vezane potrebno je implementirati dalje prevođenje.
Svi metapodaci ostaju u istom obliku u ovoj fazi kompilacije.
% Ideja je da ...
Na kraju izbora instrukcija sve instrukcije \verb|dbg.outlined| će biti zamenjene novom MIR instrukcijom \verb|DBG_OUTLINED| sa istim parametrima i debag lokacijom.
...
One se takođe pretvaraju u čvorove usmerenog acikličnog grafa ali umesto da se ubace u taj graf, one su ostavljene po strani u odvojenim strukturama podataka. % (zajedno sa SDDbgValue i SDDbgLabel)
...
% Zato što se metapodatak \verb|outline_id| čuva u mapi čiji je ključ instanca klase \verb|SDNode|, zamenom nekog čvora će se izgubiti ta veza osim ako se taj podatak eksplicitno prenese.
...
% Proizvođači kompilatora mogu da dodaju svoja proširenja standarda sve dok ne menjaju vrednosti propisane standardom DWARF.
% Kako se etikete i atributi mapiraju na celobrojne numeričke vrednosti, proizvođač može da koristi jedan slobodno izabran opseg vrednosti sve dok se ne preklapa sa opsegom nekog od drugih kompilatora.
...
Na početku obrade funkcije instanca klase \verb|DebugHandlerBase| prolazi kroz k\^od i pamti sva pojavljivanja unutrašnjih instrukcija za debagovanje.
Ta funkcionalnost je proširena da isto to uradi i za instrukcije \verb|DBG_OUTLINED|.
Radi lakšeg budućeg pristupa, one se grupišu po njihovom drugom argumentu koji predstavlja metapodatak \verb|outline_id| instrukcije poziva. % (DbgOutlinedInstrMap DbgOutlines)
...
% Proizvođači kompilatora mogu da dodaju svoja proširenja standarda sve dok ne menjaju vrednosti propisane standardom DWARF.
% % TODO. npr DWARF, GNU datoteke, dodavenje podrske za druge kompilatore
% Kako se etikete i atributi mapiraju na celobrojne numeričke vrednosti, proizvođač može da koristi jedan slobodno izabran opseg vrednosti sve dok se ne preklapa sa opsegom nekog od drugih kompilatora.
...
% TODO: Primer asemblerskih debag informacija
...
% Nacin ucitavanja prolaskom kroz debug info stablo
...
% Ovde je dodata podrška za proveru lokacija i iz nove tabele.
% Iz postojeće i nove tabele linija se dohvata red sa lokacijom najbližom prosleđenoj ().
...
% Ono se radi tako što se iz roditeljskog stek okvira dohvate potrebne informacije.
...
% Povratkom u roditeljski stek okvir lako je pronaći adresu.
...
% To se radi klasično, traženjem ogovarajućeg CallEdge i proverom IsOutlined. Novitet je što kako funkcija nije pozvana, return adresa
...
Ovaj identifikator se koristi za izmenu svojstava tačaka prekida i prikazuje se u okviru poruke korisniku kada se program zaustavi usled dostizanja jedne od njih.
% Primer
...
Primer takve poruke je prikazan u listingu \ref{lst:breakpoint_example}.

\begin{listing}[!ht]
\begin{minted}[breaklines, fontsize=\small]{text}
  * thread #1, name = 'test', stop reason = breakpoint 1.1
\end{minted}
\caption{Primer poruke koju prikazuje debager LLDB prilikom zaustavljanja na tački prekida}
...
\begin{figure}[!ht]
  ...
  \caption{Vizuelni prikaz odnosa izdvojenog koda u izvornom kodu i izvršnom fajlu}
...
% Trenutno se slični tipovi podataka ručno prenose prilikom zamene čvorova.
...
% Za implementaciju je koriščena infrastruktura LLVM, ali se ideja rešenja može preneti i na druge kompilatore i debagere.
...
% Predloženo rešenje je moguće implementirati i u drugim kompilatorima i debagerima.
% Nažalost kompilator GCC trenutno ne podržava optimizaciju izdvajanja koda pa ovo rešenje nije primenjivo na njega, ali se u debager GDB može dodati podrška za proširenje formata DWARF što bi poboljšalo iskustvo prilikom debagovanja programa prevedenog kompilatorom LLVM.
...
U poglavlju \ref{sec:compiler_impl} je pomenut problem sa održavanjem metapodatka \verb|outline_id| prilikom izbora instrukcija.
Njegovim rešavanjem, bilo u opštem ili pojedinačnim slučajevima, dodatno bi se smanjila količina preskočenih naredbi prilikom koračanja u debageru.
Zatim, trenutno postoji podrška za čuvanje debag lokacija samo iz prolaza \verb|IROutliner|.
...
% predloženo rešenje je moguće implementirati i u drugim kompilatorima i debagerima kao što su \textit{GCC} i \textit{GDB}.
