...
% TODO Predstavljanje sledecih poglavlja
% ------------------------------------------------------------------------------
...
% Kako je softver postajao slozeniji nastala je potreba za opisivanjem naprednih koncepata u programskom kodu i tako su nastali programski jezici viseg nivoa (high-level lanugage).
% Prevodjenje programa napisanih na visem jeziku je znatno komplikovanije nego jednostavna zamena mnemonika u slucaju asemblera. Tako su se razvili napredniji alati, kompajleri.

% Računari izvršavaju softver napisan u mašinskom kodu.
% Prva apstrakcija mašinskog koda nastala je u vidu asemblerskog koda.
% On je uveo mnemonike za mašinske instrukcije što je znatno poboljšalo čitljivost koda.
% Kako računar još uvek razumeju samo mapinski kod, nastali su posebni alati, asembleri, koji vrše jednostavne zamene menmonika i operanada sa odgovarajućim mašinskim kodom.
...
% Potreba za pisanjem kompleksnijeg softvera dovela je do nastanka viših programskih jezika (eng.~{\em high-level programming language}).
...
% Takodje, viši programski jezik nije vezan za konkretnu arhitekturu računara.


% Skoro sav savremeni softver je napisan korišćenjem viših programskih jezika (eng.~{\em high-level programming language}).
% Ti jezici omogućavaju programerima da koriste složenije koncepte i već poznate algoritme i strukture podataka bez potrebe da sami implementiraju sve iz početka.
% % Pre pojave visih programskih jezika, programiranje se radilo u asemblerskom kodu.
% Pojava jezika višeg nivoa omogućila je nastanak kompleksnijeg softvera.
...
% % Kljucnu ulogu imaju kompajleri

% TODO spojiti sa prethodnom glavom
...
Kako postoje već dugo vremena i čine ključne alate za razvoj softvera, kompajleri su tema brojnih istraživanja.
Kao rezulat tih istraživanja dobijeno je dosta informacija o dobrim načinima za struktuiranje i implementaciju kompajlera.
% TODO: MxN problem
Moderni kompajleri se obično sastoje iz tri dela \cite{cooper2022engineering}:
\begin{itemize}
  \item Prednji deo -- Analizara i prevodi izvorni k\^od na neku internu reprezentaciju. Pritom vrši leksičku, sintaksičku i semantičku analizu nad kodom. Ukoliko korisnički k\^od nije ispravan, kompilacija se završava ovde.
  \item Optimizator -- Optimizuje internu reprezentaciju kako bi se postigao neki cilj. Može da radi u jednom ili više prolaza.
  \item Zadnji deo -- Prevodi validnu internu reprezentaciju do programa koji može da se izvrši na ciljnoj mašini.
\end{itemize}
...
Svaki dobar kompajler ipak sadrži ovaj deo, i on je možda čak njegov najveći deo.
...
Kompajleri osim prevođenja izvornog koda do mašinskih instrukcija takođe mogu da ga modifikuju za bolje performanse sve dok se optimizovani k\^od isto ponaša kao originalni.
...
\verb|O0| je podrazumevani nivo ukoliko nijedan drugi nije specificiran i označava da se k\^od uopšte ne optimizuje.
...
% Release se koristi -O2/3, dok se za debagovanje koristi -O0
...
% Korak dalje ide nivo \verb|Oz| koji dodatno optimizuje za velicinu.
...
Cilj projekta je bio proučavanje tehnika kompajliranja u SSA obliku (eng.~{\em Static Single Assignment}) koja podržava statičku i dinamičku kompilaciju proizvoljnih programskih jezika.
...
Danas, projekat pokriva veliki broj biblioteka i alata za programiranje niskog nivoa \cite{llvm}.
...
% TODO bilo bi bolje da je ovo konkretno vezano za clang
...
% Primer nevalidne semantike
Primer provere koja se vrši na ovom nivou je da li je korišćena promenljiva deklarisana pre toga.
% Gradi se AST
...
Još uvek je moguće da dođe do greške prilikom povezivanja zbog ne postojanja definicija nekih funkcija, ali će kompilacija do asemblerskog ili objektnog koda biti uspešna.
...
Svaka funkcija se sastoji od niza osnovnih blokova, a svaki blok od niza instrukcija.
...
Takođe, početak funkcije započinje prvi osnovni blok.
...
Ukoliko isntrukcija vraća vrednost, on se smešta u novi virtuelni registar.
...
!3 = !{!"clang version 15.0.7 (Fedora 15.0.7-2.fc37)"}
\end{minted}
\caption{}
...
Prolazi se dele na analize i transformacije.
...
\begin{figure}[!ht]
\begin{minted}[]{text}
  opt -passes="mem2reg,instnamer" -o input_opt.ll input.ll
\end{minted}
\caption{}
...
Ona je dosta bliža ciljnoj arhitekturi i sastoji se od instrukcija te arhitekture.
...
\begin{itemize}
  ...
  \item \verb|SelectionDAG| --- Podrazumevana implementacija izbora instrukcija na većini arhitektura.
  ...
  \item \verb|FastIsel| --- Pristup izboru instrukcija koji radi veoma brzo, ali generiše neoptimizovan k\^od. Ne podržava spuštanje svih ilegalnih instrukcija i u tim situacijama se oslanja na \verb|SelectionDAG|.
\end{itemize}
...
\verb|SelectionDAG| je dobio ime po načinu reprezentacije koda za vreme izbora instrukcija.
...
Kontrukcija se radi prolaskom kroz LLVM međukod pomoću klase \verb|SelectionDAGBuilder| koja implementira obrazac "posetilac". % cite GoF
Za svaku instrukciju se kreira čvor na osnovu njenog koda.
...
On se dodatno optimizuje zamenom grupe čvorova sa jednostavnijim grupama uz pomoć algoritama za uparivanje stabala.
...
% MIR prolazi
I na ovom nivou postoje optimizacioni prolazi.
...
Na ovom mestu se završava zadatak kompajera.
Dalje, ukoliko je emitovan asemblerski k\^od on se prevodi do objektnog koristeći asmebler.
Objektni k\^od se povezuje sa bibliotekama i drugim objektnim kodom pomoću povezivača (eng.~{\em linker}) i dobija se izvršni fajl.
...
Komanda za prevođenje optimizovanog mašinski nezavisnog međukoda do asemblerskog fajla je prikazana u listingu \ref{lst:llc_command}.

% primer llc komanda
\begin{figure}[!ht]
  \begin{minted}[breaklines]{text}
    llc -filetype=asm -mtriple="x86_64-unknown-linux-gnu" -o output.s input_opt.ll
  \end{minted}
  \caption{}
...
% Prilikom programiranja bilo kog većeg projekta, neminovno je da ce doći do propusta.
% Ti propusti se popularno nazivaju bagovi. % i mogu se ispoljiti na više načina.
% % U zavisnosti od komponente programa posledice propusta mogu da budu vise ili manje ozbiljne.
% Bagovi se odnose na bilo kakvo ponašanje programa koje nije u skladu sa namerom programera.

% Svakako, oni trebaju biti popravljeni pre objavljivanja verzije softvera.
% % Debagovanje je proces trazenja, analiziranja i popravljanja bagova u programu.
% Proces traženja, analiziranja i popravljanja bagova u programu se naziva debagovanje.
% Postoji nekoliko načina debagovanja.
% Osnovni vid je debagovanje štampanjem (eng.~{\em print debugging}).
% Debagovanje stampanjem podrazumeva izmenu izvornog koda tako da ispisuje poruke i vrednosti promenljivih u glavnim koracima dela koda za koju programer smatra da su razlog za postojanje greske.
% Za sitnije greske ovo je sasvim prihvatljiv nacin resavanja.
% Naravno, mane pristupa su potreba za ponovnom kompilacijom celog programa svaki put kada se dodaje ili menja ispis.
% Kompilacija velikih projekata moze da traje bas dugo, tako da se ovaj pristup ne isplati uvek.
% Alternativa je koriscenje posebnog alata namenjenog za ovu situaciju.
% Debager je softverski alat koji olaksava proces debagovanja.
% % mogucnosti debagera
% Neke osnovne mogucnosti (feature) debagera su koracanje kroz program, tacke prekida, evaluiranje vrednosti promenljivih ili izraza itd.

% Iako postoje kao posebni alati koji se koriste iz komandnog prozora, debageri su cesto integrisani u razvojna okruzenja.
...
% Za uspešno debagovanje programa potrebna je podrška kompajlera da sačuva informacije o izvornom kodu do izvršnog fajla.
% Pored mašinskog koda, kompajler takođe, ako mu je zatraženo, generiše i dodatne informacije koje se koriste za debagovanje.
% One uključuju mapiranja mašinskih instrukcija u lokacije u izvornom kodu, imena funkcija i promenjivih itd.
% Kompajler LLVM generiše ove podatke dodavanje zastavice \verb|-g|.

% % Ove informacije se nalaze u posebnim segmentima, van uobičajenih segmenata (text, data, bss...).
% Izvršni fajl je normalno podeljen u nekoliko delova koji se nazivaju segmenti.
% Najbitniji među njima su tekst segment gde se nalaze instrukcije, segment inicijalizovanih podataka i segment neinicijalizovanih podataka.
% Pored njih mogu da postoje i druge sekcije, na primer za razmotavanje u slučaju greške.
% % ne smeju da menjaju kod koji se izvrsava
% Informacije za debagovanje ne diraju nijedan od prethodnih segmenata već definišu nove kako ni bi menjali postojeće podatke.
% Tačan broj i naziv novih segmenata zavisi od formata informacija za debagovanje koji se koristi.

% Postoji vise razlicitih formata za zapisivanje informacija za debagovanje.
% Najpopularniji medju njima su CodeView za Windows sisteme i DWARF za sisteme UNIX tipa.

% % Odnos optimizacija i debag informacija
% Generisanje informacija za debagovanje nailazi na problem kada se uključe kompajlerske optimizacije.
% Što se više krajnji program razlikuje od izvornog koda postaje teže očuvati informacije.
% U nekim slučajevima je čak potpuno nemoguće.
...
% DWARF je standardizovan format za zapisivanje informacija za debagovanje.
% Dizajniran da radi uz format ELF za biblioteke, izvršni i objektni k\^od, ali podržava i druge formate.
...
% Ovaj format se koristi na UNIX sistemima koji uključuju Linux distribucije i macOS.

% Sve informacije za debagovanje su raspoređene u nove sekcije izvršnog fajla.
% Te sekcije počinju prefiksom \verb|.debug_|.
% % propisuje sekcije: .debug_info .debug_frame .debug_line
% \begin{itemize}
%   \item \verb|.debug_info|
%   \item \verb|.debug_line|
%   \item \verb|.debug_frame|
%   \item \verb|.debug_abbrev|
% \end{itemize}

% % etikete i atributi
% Podaci su predstavljeni pomoću osnovnih jedinica (eng.~{\em Debug Information Entry, DIE}) organizovanim u drvoliku strukturu.
% Svaka jedinica ima etiketu (eng.~{\em tag}) koja označava njen tip, dodatne informacije predstavljene atributima i može da ima potomke.
% Standard propisuje validne etikete i atribute koje moraju da postoje za te etikete.

% % Format je veoma kompresovan. Sve se cuva kao litarli sa propisanim velicinama, ne postoje nikakva imena.
...
% % https://lldb.llvm.org/
% % LLVM Cookbook, chapter 9, Using LLDB
% % https://lldb.llvm.org/use/tutorial.html

% LLDB je jedan od potprojekata koji se razvija kao deo projekta LLVM \cite{lldb}.
...
% % Za jezike C, Objective-C and C++

% % On radi i na kodu koji je generisan kompajlerom GCC.
% % Kombinacija kompajlera i debagera nije bitna sve dok rade u istom formatu, ali se informacije koje kompjaler generise i debager razume potencijalno razliciti.

% % Podrzava Windows, Mac, Linux

% % LLDB je podrazumevani debager integrisan u Apple Xcode integrisano razvojno okruzenje, i podrzan je od strane drugih okruzenja i debagera ukljucujuci JetBrains CLion, QtCreator, Eclipse i VSCode.
% % Takodje ima TUI - https://peeterjoot.com/2019/08/26/the-lldb-tui-text-user-interface/
...
Ova optimizacija je posebno korisna za uređaje sa malom količinom memorije, najčešće uređajima sa ugrađenim računarom (eng.~{\em embedded devices}) poput pametnih satova, mp3 plejera ili urađaja zasnovanih na razvojnom sistemu Arduino.
...
Takođe, osim samog poziva, stek mora da bude pripremljen za poziv i parametri premešteni na odgovarajuća mesta.
...
Ukoliko se ovaj k\^od prevede do asemblerskog koda komandama prikazanim u listingu \ref{lst:outlining_compile_instructions} tako da se pokrene samo optimizacija izdvajanja koda, rezultujući k\^od je prikazan u listingu \ref{lst:outline_program_asm}.
...
Još uvek postoji jedan kandidat iz grupe koji predstavlja telo izdvojene funkcije.
On se sabira sa procenama cena svih poziva izdvojene funkcije i cenama dodatnih instrukcija za kreiranje novog okvira za funkciju.
...
Ona je znatno kompilkovanija nego mašinski nezavisna varijanta, ali pruža više mogućnosti za izdvajanje.
...
Dobitak se računa slično kao na mašinski nezavisnom nivou, kao razlika bez i sa uključenom optimizacijom.
Kako je ovaj međukod dalji od krajnjeg koda, procena postaje teža jer se oslanja na manje precizne podatke.
Cena bez optimizacije se ovaj put računa kao suma procena veličina svih kandidata.
Cena sa optimizacijom sadrži prosečnu veličinu kandidata i procene veličina instrukcija za smeštanje i učitavanje potrebnih argumenta.
...
Za izbor instrukcija se koristi \verb|SelectionDAG| implementacija, tako da će čuvanje podataka raditi za sve podržane arhitekture.
...
Unutrašnje funkcije su funkcije koje su ugradjene u sami kompajler takve da on može da ih implementira na optimalan način \cite{sarda2015llvm_essentials}.
...
Imena svih unutrašnjih funkcija počinju prefiksom "\verb|llvm.|", on će u nastavku biti izostavljen.
...
% % SelectionDAG uvod?
% SelectionDAG prevodi LLVM međukod u reprezentaciju u obliku usmerenog acikličnog grafa, a svaka instrukcija postaje čvor u tom grafu.
% % TODO: koji oni?
...
% Ideja je da ...
Na kraju iybora instrukcija sve \verb|dbg.outlined| instrukcije će biti zamenjene ekvivalentnom MIR instrukcijom \verb|DBG_OUTLINED| sa istim parametrima i debag lokacijom.
...
Na primer, prilikom spuštanja mašinski nezavisne instrukcije poziva funkcije na mašinski zavisan nivo za arhitekturu X86 pravi potpuno novi čvor.
Zato što se \verb|outline_id| čuva u mapi ciji je ključ \verb|SDNode|, zamenom nekog čvora će se izgubiti ta veza osim ako se taj podatak eksplicitno prenese.
% i metapodatak nece biti emitovan uz zamenjenu instrukciju.
Pomenuti primer je popravljen u implementaciji, ali još uvek postoji veliki broj slučajeva kada će se ovaj problem ispoljiti.
...
Uključivanje informacija za debagovanje ne sme da menja krajnji k\^od tako da nijedna instrukcija neće biti dodata u tekst segment programa.
Ova implementacija podržava samo ispisivanje u formatu DWARF. % TODO pomeriti?
...
% Tu spadaju lokacija u izvornom kodu koja se dobija iz debag lokacije i adresa odgovarajuće instrukcije u tekst segmentu.
% Adrese se pojavljuju u kodu tek prilikom linkovanja tako da umesto konkretnih vrednosti adresa ovde se koriste labele koje će biti postavljene ispred odgovarajućih instrukcija.
...
Druga stvar koja se radi takođe na početku funkcije je zahtevanje za kreiranje labela ispred instrukcija u funkcijama generisanim izdvajanjem koda koje imaju \verb|outline_id|.
...
\end{itemize}
Na kraju obrade modula sigurno su spremni svi neophodni podaci iz svih funkcija korisničkog koda.
...
Te vrednosti se direktno prenose u objektni i izvršni fajl.
% npr. .byte 1 .byte 16 .long 4 ...
% TODO: Primer asmeblerskih debag informacije
...
To uključuje ispisivanje tačnih mesta u izvornom kodu kada se proces zaustavi, podršku za izvšavanje korak po korak i postavljanje tačaka prekida.

% TODO: ima li jos nesto da je potrebno opisati ovde?
...
Obično oba ova mapiranja radi tabela linija (eng.~{\em line table}),
ali za izdvojene instrukcije ta dva smera su nezavisno implementirana na različite načine zato što su im potrebni različiti podaci a koji nisu svi dostupni u vreme kompilacije.
...
Kao i u ranijim poglavljima, podržani formati su implementirani kao priključci (eng.~{\em plug-ins}).
...
Stanje programa podrazumeva jedinicu kompilacije, funkciju i lokacija u izvornom kodu.
...
% U tim situacijama 
Oba načina se mogu koristiti za proveru ispravnosti implementacije.
...
U suprotnom koristi se nova implementacija.
% Iz prethodne glave postoji podrška za mapiranje adrese na liniju u izvornom kodu.
...
% Stek okviri predstavljaju najprecizniji vid konteksta.
...
Nit programa ima dostupnu listu stek okvira koji postoje u tom trenutku.
Povratkom u roditeljski stek okvir lako je pronaći tu adresu.
% Kada je pronađena povratna adresa.
Konačna lokacija je dobijena iz \verb|CallEdge| instance dobijene iz tog roditeljskog stek okvira pomoću metode \verb|Function::GetCallEdgeForReturnAddress| sa pronađenom adresom povratka kao parametrom.  
...
Između dva koraka korisnik može da pogleda vrednosti relevantnih promenljivih i priveri da li se one poklapaju sa očekivanim vrednostima.
% Step over, into
% Step inst vec radi
...
\begin{itemize}
  ...
  \item \verb|step-inst| --- Izvršava jednu instrukciju izvršnog fajla
\end{itemize}
U postojećoj implementaciji jedino \verb|step-inst| komandom je moguće stati na izdvojenu instrukciju.
Druge komande, kako se oslanjaju na broj reda u izvornom kodu, će ih uvek preskočiti.
Cilj ovog poglavlja je prepraviti i njih da rade u skladu sa korisnikovim očekivanjima.

% ThreadPlan objasnjenje
% TODO: Napomenuti LLDB
...
Svaka nit ima svoj stek planova i njemu se može pristupiti \verb|GetProcess().GetThreadPlans(GetID())| iz trenutne niti.
...
Taj plan se ona upita da li treba da se vrati kontrola korisniku funkcijom \verb|ShouldStop|.
Ovde se ažurira logika ukoliko je potrebno da se program zaustavi na nekom sledećem mestu.
Posle izvršavanja plana proverava se da li je on završio pozivom funkcije \verb|MischiefManaged|.
...
Fokus ovog poglavlja su implementacije \verb|ThreadPlanStepInRange| i \verb|ThreadPlanStepOverRange| koje se koriste redom za \verb|step-into| i \verb|step-over| komande.
...
Nije moguće odmah postaviti tačku prekida direktno na konačnu lokaciju zbog mogućih grananja u programu.
Razlika između planova je što \verb|step-into| prilikom zaustavljanja na instrukciji poziva funkcije dodaje novi plan za ulazak u funkciju dok \verb|stop-over| to ne radi.

Ideja rešenja je da se osim postojeće tačke prekida uvede još jedna koja se uvek postavlja na seldeću izdvojenu instrukciju.
...
Tačka prekida (eng.~{\em breakpoint}) je mesto u programu koje kada treba da se izvrši u debageru izaziva pauziranje izvršavanja i vraćanje kontrole korisniku.
...
Tačno je da isti ovaj proces se koristi i za postavljanje tačke prekida po imanu funkcije ali kako izdvojeni k\^od ne može da sadrži deklaraciju funkcije, na njega ne utiče dotični problem.
Na sreću, potreban mehanizam je već ostvaren dodatnom tabelom linija za izdvojene instrukcije.
...
\end{figure}
Ukoliko korisnik postavi tačku prekida bilo na red 17, do prekida će doći i prilikom izvršavanja linije 22 i obrnuto.
...
Dolaskom na tačku prekida vrši se provera da li treba vratiti kontrolu korisniku kroz funkciju \verb|ShouldStop| klase \verb|BreakpointLocation|.
U ovom trenutku je potrebno proveriti da li se lokacija poklapa sa lokacijom koja odgovara trenutnom stanju procesa.
Način dobijanja trenutne lokacije je isti kao i u prethodna dva poglavlja, uz pomoć trenutne adrese i konteksta.
Nigde u implementaciji se ne nalazi zavisnost na određeni način određivanja tačaka prekida tako da bi trebalo da funkcioniše 
...
Modifikovanu verziju kompajlera i debagera je potrebno detaljno testirati u različitim mogućim situacijama kako bi se postarali da se prvenstveno ne smanjuje funckionalnost u odnosu na nemodifikovanu verziju, a zatim i da se zapravo unapređuje debagovanje izdvojenog koda.
...
Ti testovi se pokreću koristeći pomocni alat \verb|lit| (skraćeno za {\em LLVM Integrated Tester}) i sastoje se od fajlova koji sadrže k\^od u nekom formatu i ugradjene direktive u vidu komantara.
Direktive mogu da budu ili komande koje se pokrecu za izvrsavanje testa ili direktive koje zadaju tvrdnje koje test proverava.
Testovi se pokreću 

% Testovi
Pokrenuti su svi testovi uključeni u LLVM projekat. % Da li se nesto pormenilo prilikom generisanja debug info
Većina testova je uspešno izvršena bez problema. To je bilo i očekivano zato što veliki broj testova i ne radi sa uklječenim izdvajanjem koda.
Mali deo testova je neuspešno izvršen zato što proverava specificne informacije za debagovanje koje su izmenjene modifikovanim kompajlerom.
...
Dotični test je prevremeno zaustavljao zbog neispunjene tvrdnje da poziv funkcije koja moze da bude umetnuta unutar funkcije koja ima inforamcije za debagovanje mora da ima lokaciju.
Za taj slučaj je dodata posebna podrška.
Za pozive koji ispunjavaju opisani uslov se postavlja veštačka debag lokacija.

% Kada je provereno da implementacija ne dovodi do regresija
Ispravkom poslednjeg testa, nije uočeno više problema i svi testovi su se uspešno izršavali.
...
Demonstracija će biti prikazana na istom primeru prestavljenim ranije u listingu \ref{lst:outline_program_example}.
...
Debager testovi pokreću debager nad programom sa izdvojenim kodom i proveravaju ispravnost lokacije, da li se program zaustvio na očekivanim mestima i da li je uspešno postavljena tačka prekida.
